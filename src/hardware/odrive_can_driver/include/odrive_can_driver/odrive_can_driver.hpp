...
private:
    ...
    // トルク情報
    float torque_target_ = 0.0f;
    float torque_estimate_ = 0.0f;
...
private:
    ...
    void send_traj_vel_limit_command(uint8_t node_id, float vel_limit);
    void send_clear_errors_command(uint8_t node_id, uint8_t identify = 0);
